...
app.layout = html.Div([
    ...
    dcc.Dropdown(
        id="parameter-dropdown", options=[], value=None, placeholder="Select flight option"
    ),
...
def update_dropdown_selection(selected_option: int):
    ...
    options = [
        (
            {
                "label": f"{p.departure_location}-{p.arrival_location}, {p.departure_date}\t ||| \t"
...
def update_plot(selected_option: int):
    if not selected_option:
        return go.Figure()
    session = Session(engine)
    flight_data = session.query(Flight).filter(Flight.parameters_id == selected_option).all()
    session.close()
    figure = go.Figure(
        data=go.Scatter(
            x=[flight.created_at.isoformat(timespec="minutes") for flight in flight_data],
