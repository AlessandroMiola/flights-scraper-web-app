...
def update_params_and_call_scraper(
    ...
    try:
        ...
        return templates.TemplateResponse(
            name="flight-details/home.html", context={"request": request, "exceptions": exceptions}
        )


@router.get(
    "/all-flights", response_model=dict[int, list[FlightShow]], status_code=status.HTTP_200_OK
)
def get_all_flights_details(db: Session = Depends(get_db)):
    all_flights_data = get_all_flights_data(db=db)
    if not all_flights_data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No flight data available."
        )
...
def delete_flight_details(params_id: int, db: Session = Depends(get_db)):
    ...
    if msg.get("error"):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=msg.get("error"))
